...
void copy_data(uint8 * test_images, uint8 * test_images_char)
{
		...
		for(int c = 0; c<CHANNEL; c++) {
			for(int h = 0; h<HEIGHT; h++) {
				for(int w = 0; w<WIDTH; w++) {
					int temp = (uint8)(((float)test_images_char[
						n * CHANNEL * HEIGHT * WIDTH + 
						c * HEIGHT * WIDTH + 
						h * WIDTH + 
						w
					] - mean[c]) / var[c] / 0.016631 + 104);
					test_images[
						n * CHANNEL * HEIGHT * WIDTH + 
						c * HEIGHT * WIDTH + 
						h * WIDTH + 
...
int main(void)
{
    ...
    uint8 * test_images = (uint8*)malloc(sizeof(uint8)*TEST_NUM*CHANNEL*HEIGHT*WIDTH);
    // copy_data(test_images, test_images_char);
	test_images = test_images_char;
